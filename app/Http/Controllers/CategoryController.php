...
class CategoryController extends Controller
{
    public function index()
    {
        ...
        if($role->hasPermissionTo('categories-index')){            
            ...
            return view('category.create',compact('lims_categories', 'lims_category_all', 'all_permission'));
        }
        else
    ...
    public function categoryData(Request $request)
    {
        ...
        if(!empty($categories))
        {
            foreach ($categories as $key=>$category)
            {
                ...
                if(!in_array("categories-edit", $request['all_permission']) && !in_array("categories-delete", $request['all_permission'])) {
                    $nestedData['options'] = '---';
                } else {
                $nestedData['options'] = '<div class="btn-group">
                            ...
                            if(in_array("categories-edit", $request['all_permission']))
                $nestedData['options'] .= '
                                <li>
                ...
                $nestedData['options'] .= '
                                <li class="divider"></li>';
                            if(in_array("categories-delete", $request['all_permission']))
                $nestedData['options'] .= \Form::open(["route" => ["category.destroy", $category->id], "method" => "DELETE"] ).'
                                <li>
