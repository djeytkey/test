    ...
    <div id="receipt-data">
        <div class="centered">
            @if($general_setting->site_logo)
                <img src="{{url('public/logo', $general_setting->site_logo)}}" width="150" style="margin:10px 0;/*filter: brightness(0);*/">
            @endif
        ...
        <p>{{trans('file.Date')}}: {{date(config('date_format'), strtotime($lims_sale_data->created_at->toDateString()))/*date($lims_sale_data->created_at)*/}}<br>
        ...
        <table class="table-data">
            <tbody>
                ...
                <tr>
                    <td colspan="2">
                        {{$product_name}}
                        <br><small>{{$product_sale_data->qty}} x {{number_format((float)($product_sale_data->total / $product_sale_data->qty), 2, '.', ' ')}}</small>
        ...
        <div class="centered" style="margin:30px 0 50px">
            <small>{{$general_setting->site_title}} &copy; {{ date("Y") }}</strong></small>
        </div>
    </div>
</div>
